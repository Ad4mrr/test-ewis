...
function BlogsPageAdmin() {
    ...
    const fetchBlogs = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/blogs?page=${currentPage}&limit=5`, {
                method: 'GET',
                credentials: 'include',
            });
            if (!response.ok) {
            ...
            <AuthWrapper>
                ...
                <div className="w-full flex flex-col items-center bg-primarygray py-10">
                    ...
                    <div className="overflow-x-auto w-full px-20">
                        ...
                        {loading ? (
                            <p className='font-Poppins text-lg font-medium text-center py-6'>Loading blogs...</p>
                        ) : error ? (
                            <p>Error: {error}</p>
                        ) : blogs.length === 0 ? (
                            ...
                            <div className='bg-white rounded-xl overflow-hidden flex flex-col'>
                                <table className="min-w-full ">
                                    ...
                                    <tbody>
                                        {blogs.map(blog => (
                                            <tr key={blog._id} className="border-b border-primarysh font-Poppins text-sm text-left">
                                                <td className="px-4 py-2">
                                                    {blog.cover && (
                                                        <img
                                                            src={`http://localhost:5000/${blog.cover}`} // Ensure the correct path
                                                            alt={blog.title}
                                ...
                                {Array.from({ length: totalPages }, (_, index) => (
                                    <li key={index}>
                                        <button
                                            ...
                                            className={`py-2 px-3 leading-tight ${currentPage === index + 1 ? 'text-primary bg-primarygray font-Poppins font-semibold' : 'text-blackFooter bg-white font-Rubik font-semibold'} border border-gray-300 hover:bg-gray-100 hover:text-gray-700`}
                                        >
                                            {index + 1}
                                        </button>
                                    </li>
