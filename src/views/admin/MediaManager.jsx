...
const MediaManager = () => {
    ...
    const renderFilePreview = (file) => {
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(fileType)) {
            return <img src={file.url} alt={file.name} className="file-preview-image rounded-xl z-10" />;
    ...
    return (
        <AuthWrapper>
            ...
            <div className="w-full bg-primarygray py-10 flex flex-col items-center">
                <h1 className='text-center font-Poppins font-bold text-2xl mb-5'>Media Manager</h1>
                    ...
                    {files.length === 0 ? (
                        ...
                        <div className='basis-1 flex flex-wrap justify-center items-center gap-2'>
                            {files.map((file, index) => (
                                <div key={index} className="p-4 basis-1/4 bg-white rounded-xl overflow-hidden ">
                                    <div className='font-Rubik font-medium text-sm transition-all duration-300 hover:scale-105'>{renderFilePreview(file)}</div>
                                </div>
                            ))}
                        </div>
                    )}
