...
function SingleJob() {
    ...
    return (
        <>
            ...
            <NavBar />
            <div className="py-12">
                <div className="max-w-7xl mx-auto px-4 font-Poppins flex flex-col items-start gap-5">
                    <div className='flex flex-col gap-2'>
                        <div className='flex flex-row items-center justify-between'>
                            <h1 className="font-bold text-3xl capitalize">{job.jobTitle} </h1>
                            <button onClick={openModal} className="bg-primary text-white px-7 py-2 rounded-3xl flex flex-row items-center justify-center gap-2">
                                <span>Apply Now</span>
                        ...
                        <div className='flex flex-wrap items-start justify-start gap-3'>
                            {job.skills.map(skill => (
                                <span key={skill} className='bg-primarygray px-4 py-1 rounded-full ring-1 ring-white text-xs'>{skill}</span>
                            ))}
                        </div>
                    ...
                    <p className='font-Poppins text-justify'>{parse(job.jobDescription)}</p>
                    <button onClick={openModal} className="bg-primary text-white px-7 py-2 rounded-3xl mt-5 flex flex-row items-center justify-center gap-2">
                        <span>Apply Now</span>
                        <div className='w-5'><PaperClipIcon /></div>
                    </button>
                </div>
                    ...
                    <form onSubmit={handleApply} className="flex flex-col gap-4">
                        {isSubmitting ? (
                            <div className="flex justify-center items-center">
                                <p className='font-Poppins text-lg font-medium text-center py-6 capitalize'>Submitting your Resume...</p>
                            </div>
                        ) : (
                            <>
                                <input
