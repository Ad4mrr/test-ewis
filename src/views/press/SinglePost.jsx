...
export default function SinglePost() {
    ...
    if (!post) return <div className='w-screen h-screen flex flex-col items-center justify-center font-Poppins text-2xl bg-primary text-white'><span>Loading...</span></div>;
        ...
        <>
            ...
            <div className="max-w-7xl mx-auto bg-fixed px-5 lg:px-0 py-20">
                <div className='w-full flex flex-row items-start justify-start'>
                    <div className="p-5 basis-3/4 gap-5 flex flex-col">{loading ? (
                        <>
                            <p className='text-blackFooter font-normal font-Poppins text-justify'>Loading ...</p>
                        </>
                    ) : (
                        <>
                            <h1 className="text-3xl font-bold font-Poppins capitalize text-left">{post.title}</h1>
                            {post.cover && (
                                <img
                                    src={`http://localhost:5000/${post.cover}`} // Adjust port if needed
                                    alt={post.title}
                                    className="object-cover w-full rounded-2xl"
                                />
                            )}

                            <p className="text-blackFooter font-normal font-Poppins text-justify ">{post.summary}</p>
                            <div className='text-blackFooter font-normal font-Poppins text-justify' dangerouslySetInnerHTML={{ __html: post.content }} />
                        </>
                    )}
                    </div>
                    <div className="p-5 basis-1/4 flex flex-col gap-5">
                        <h2 className="text-2xl font-bold font-Poppins border-b-2">Recent Posts</h2>
                        {loading ? (
                            <p className='text-blackFooter font-normal font-Poppins text-justify'>Loading recent posts...</p>
                        ) : error ? (
                            <p>Error: {error}</p>
                                    ...
                                    <div key={recentPost._id} className="mb-4">
                                        ...
                                        >
                                            ...
                                            <div className="overflow-hidden rounded-xl w-full">
                                                <img
                                                    src={`http://localhost:5000/${recentPost.cover}`} // Ensure the correct path
