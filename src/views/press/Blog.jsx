...
export default function Blog() {
    ...
    useEffect(() => {
        const fetchBlogs = async (page = 1) => {
            try {
                const response = await axios.get(`http://localhost:5000/api/blogs?page=${page}&limit=9`);
        ...
        <>
            ...
            <div className="max-w-7xl mx-auto bg-fixed px-5 lg:px-0 py-20">
                <div className="flex flex-col lg:flex-row items-start justify-center h-full gap-5">
                    {loading ? (
                        <p className='font-Poppins text-3xl font-medium'>Loading blogs...</p>
                    ) : error ? (
                        <p>Error: {error}</p>
                    ) : blogs.length === 0 ? (
                        <p className='font-Poppins text-3xl font-medium'>No blogs available.</p>
                    ) : (
